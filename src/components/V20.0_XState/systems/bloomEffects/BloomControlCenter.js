...
export class BloomControlCenter {
  ...
  setObjectTypeProperty(objectType, property, value) {
    ...
    const objectCount = this.objectsByType[objectType]?.size || 0;
    // ✅ RÉDUCTION LOGS : Seulement si objets existent
    if (objectCount > 0 && property === 'emissiveIntensity') {
      console.log(`🔧 BCC: ${objectType} ${property}=${value} (${objectCount} objects)`);
    }
    ...
    // ✅ RÉDUCTION LOGS : Seulement si modifié
    if (modifiedCount > 0) {
      setTimeout(() => {
        console.log(`✅ BCC: ${objectType} - ${modifiedCount} objects modified`);
      }, 5);
    }
  }
  ...
  setPostProcessParameter(parameter, value) {
    if (this.postProcessConfig[parameter] !== undefined) {
      ...
      // 🔧 ARCHITECTURE SIMPLIFIÉE : Paramètres reveal_ deprecated
      // ⚠️ SUPPRIMÉ: Paramètres bloom spécifiques pour reveal rings (impossible avec UnrealBloomPass)
      console.warn(`⚠️ BloomControlCenter: reveal_ parameter ${parameter} deprecated. Use global bloom only.`);
    }
  }
  ...
  setBloomParameter(param, value, groupName = null) {
    if (this.renderingEngine && this.renderingEngine.updateBloom) {
      if (groupName) {
        // 🔧 ARCHITECTURE SIMPLIFIÉE : Paramètres par groupe - SEULEMENT emissive 
        // ⚠️ SUPPRIMÉ: threshold/strength/radius par groupe (impossible avec UnrealBloomPass)
        
        if (['threshold', 'strength', 'radius'].includes(param)) {
          console.warn(`⚠️ BloomControlCenter: Group bloom parameter ${param} ignored. Use global bloom controls only.`);
          return false;
        }
        
        // ✅ Autres paramètres par groupe (comme emissiveIntensity) ne sont pas gérés ici
        // Ils sont gérés directement par SceneStateController → matériaux
        console.warn(`⚠️ BloomControlCenter: Group parameter ${param} should be handled by SceneStateController`);
        return false;
      } else {
  ...
  // 🔧 ARCHITECTURE SIMPLIFIÉE : Obtenir les paramètres d'un groupe (deprecated)
  getGroupBloomSettings(groupName) {
    console.warn(`⚠️ BloomControlCenter: getGroupBloomSettings deprecated. Use global bloom only.`);
    return { threshold: 0.15, strength: 0.4, radius: 0.4 }; // Valeurs globales par défaut
  }
