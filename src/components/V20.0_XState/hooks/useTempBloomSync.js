          ...
          if (bloomChanged) {
            ...
            if (sceneController) {
              ...
              if (sceneController.setMaterialParameter) {
                ...
                // 🔧 CORRECTION CRITIQUE: Forcer le rendu après mise à jour emissive
                // Problème identifié: emissiveIntensity ne s'affiche qu'après toucher strength
                if (window.renderer && window.scene && window.camera) {
                  console.log('🔧 Forcing render after emissive materials update');
                  window.renderer.render(window.scene, window.camera);
                  
                  // Double force avec timeout pour s'assurer de la prise en compte
                  setTimeout(() => {
                    if (window.renderer && window.scene && window.camera) {
                      window.renderer.render(window.scene, window.camera);
                    }
